...
```txt
55aa00000000-55aa00001000 r--p ... /path/linux_fd_print
55aa00001000-55aa00002000 r-xp ... /path/linux_fd_print
55aa00002000-55aa00003000 r--p ... /path/linux_fd_print
55aa00003000-55aa00004000 rw-p ... /path/linux_fd_print
7fff4de8d000-7fff4deae000 rw-p ... [stack]
```
...
따라서 다음 코드로 만든 지역 배열은 실제로 stack에 저장되어 있음을 확인할 수 있다.
...
일반적으로 프로세스의 메모리 영역은 다음과 비슷하게 나뉜다.
...
## 8. 왜 28,000,000바이트가 전부 출력되지 않는가?
...
그래서 요청은 28,000,000바이트였지만 실제 출력은 그보다 훨씬 작을 수 있다.
...
이 실험 코드에서 `sleep(20)`을 넣은 이유도 이 때문이다.  
